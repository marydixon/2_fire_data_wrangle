...
ndsi_ndvi <- inner_join(annual_long_ndsi,annual_long_ndvi,by="year")

ggplot(ndsi_ndvi,aes(x=mean_NDSI,y=mean_NDVI,col=site.y)) + 
  geom_point() + 
  theme_few() + 
  scale_color_few() + 
...
The burned sites may have a slight negative correlation, but any correlation if present is very weak. The unburned sites also appear to exhibit a slightly negative correlation, but still this correlation is very weak.  

The pre-burned sites do not appear correlated. The visualized data progress horizontally without an indication of either a positive or negative relationship. However, the burned site pre-burn has a few points that decrease in value indicating a possible negative correlation. The post-burned sites may have a weak and slightly negative correlation.  

The patterns appear similar between pre- and post-burn within the same treatment (burned/unburned). Ultimately, the correlations are weak between winter NDSI and summer NDVi regardless of site or treatment.None of the graphs showcase strong positive or negative correlations.
...
```
  ...
  The NDVI value corresponds to more greenness because it is a vegetation index. September has the greatest mean NDVI value for the unburned site, closely followed by August. August had the greatest mean NDVI value for the burned site, closely followed by September.  
